...
// GIS files input
	...
	int initial_population <- 1000 parameter: 'P04: Initial population';
	int population_growth <- 1 min: 0 max: 6 parameter: 'P05: Population growth rate (%)';
	float space_per_person <- 9.0 parameter: 'P06: Space for each person at home (sqm)';
	float par_unit_x <- 3.0 parameter: 'P07: Width of new units (m)';
	float par_unit_y <- 3.0 parameter: 'P08: Height of new units (m)';
	float unit_z <- 3.0 parameter: 'P09: Depth of new units (m)';
	int build_time <- 2 parameter: 'P10:Time needed for building a unit';
	int build_probability <- 100 parameter: 'P11: Probability for a new unit to be built (%)';
	int max_storeys <- 5 parameter: 'P12: Maximum number of storeys';
	float distance_between_units <- 0 parameter: 'P13: Distance between two neighbor units';
	int max_unit_per_cycle <- 10 parameter: 'P14: Maximum possible number of units build at a time';
	float shop_distance_to_road <- 20 parameter: 'P15: Max distance between the road/path and a shop';
	int iteration_one <- 1979 parameter: 'P16: Year to begin T1';
	int iteration_two <- 1989 parameter: 'P17: Year to begin T2';
	int iteration_three <- 1999 parameter: 'P18: Year to begin T3';
	int iteration_four <- 2009 parameter: 'P19: Year to begin T4';
	int stop_year <- 2016 parameter: 'P20: Year to stop simulation';
	int days_per_cycle <- 7 min: 1 max: 360 parameter: 'P21: Number of days represented by one cycle';
	string result_output <- '../results'; //parameter: 'Folder to save the output';
	int prob_to_shop <- 20 min: 0 max: 100 parameter: 'P22: Probability for home turn into shop';
	int orphanage_distance <- 50 parameter: 'P23: Max distance between orphanage and wat to turn into orphanage';
	int prob_to_orphanage <- 15 parameter: 'P24: Probability for home turn into orphanage';
...
{
	parameter "P1: Running scenario" var: running_scenario <- 1;

	// scenario 2 only 
	parameter "P2: (S2 only) Random units" var: num_init <- 10;
	// scenario 3 only
	parameter "P3: (S3 only) Initial shop percentage" var: percent_shop_random <- 50;
	output
	{
...
experiment 'Batch' type: batch repeat: 20 keep_seed: true
{
